...
void
Preprocess::process_cut_frame_pcl2(const sensor_msgs::msg::PointCloud2::ConstPtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out,
    ...
    if (lidar_type == VELO) {
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.curvature = (pl_orig.points[i].timestamp - msg->header.stamp.sec+msg->header.stamp.nanosec/1e9) * 1000;  //s to ms
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.curvature = (pl_orig.points[i].timestamp - msg->header.stamp.sec+msg->header.stamp.nanosec/1e9)* 1000.0;  //ms
    ...
    for (uint i = 1; i < valid_pcl_size; i++) {
        valid_num++;
        // pl_surf[i].curvature += msg->header.stamp.toSec() * 1000 - last_frame_end_time;
        pl_surf[i].curvature += (msg->header.stamp.sec + msg->header.stamp.nanosec/1e9) * 1000 - last_frame_end_time;
        pcl_cut.push_back(pl_surf[i]);
